function [ObjFunction,house,...
    ...
    OptimisedCaseModel_DHW(x,Wpv,Ddhw,GlobHorIrr,Text,Cimp,Cexp,nDays,tankVolume,ICs,objectiveFunction)


%This function simulates the operation of a heat pump connected to a DHW cylinder
% given a specified control. 
...
chargingTank_Control = x(1:len/2);                                          % binary: charging-tank (on/off)
heatOutput_Control =  x((len/2)+1:len)  ;                                   % continous: heat pump + electric heater delivered power (W)

% Number of time steps
% --------------------

NstepsControl = length(chargingTank_Control)    ;                           % number of times a control decision is made
...
aT     = repelem(chargingTank_Control,NstepsInputs/NstepsControl)';         % binary: charging-tank (on/off)
heatingPower  = repelem(heatOutput_Control,NstepsInputs/NstepsControl)';    % continous: heat pump + electric heater delivered power (W)
...
for n = 2:length(time)
    ...
    % Check whether hot water tank is being charged
    controls.chargingTank = aT(n);
    ...
    if house.T(3) < TminSpace
        ...
        if house.T(3) > 16+273.15
            ...
            house = house.HP_performance_part_load(Text(n));
            if isnan(house.COP)
                controls.spaceHeating = 0;
            end
        end
...
% Penalty for not meeting DHW demand
%=========================================
...
switch objectiveFunction
    case 'TotOperCost'
        ObjFunction = TotOperCost + PenaltyUBCost;
    case 'AverageSystemCOP'
        ObjFunction = - AverageSystemCOP_LB;
